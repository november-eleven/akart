...
func (t *tree) _forEach(children []*artNode, callback Callback) bool {
	for i, limit := 0, len(children); i < limit; i++ {
		child := children[i]
		if child != nil {
			if !t.forEach(child, callback) {
				return false
			}
		}
	}
...
func (t *tree) forEach(current *artNode, callback Callback) bool {
	if current == nil {
		return true
	}
	...
	switch current.kind {
	case Node4:
		if !t._forEach(current.node4().children[:], callback) {
			return false
		}

	case Node16:
		if !t._forEach(current.node16().children[:], callback) {
			return false
		}
	...
	case Node256:
		if !t._forEach(current.node256().children[:], callback) {
			return false
		}
	}
